...
from target_hubspot.utils import logger, request, SESSION, get_params_and_headers, on_giveup, giveup
...
def get_custom_schema(entity_name, config):
    return parse_custom_schema(entity_name, request(config, get_url(entity_name + "_properties")).json())

def get_v3_schema(entity_name, config):
    url = get_url("deals_v3_properties")
    return parse_custom_schema(entity_name, request(config, url).json()['results'])

def get_abs_path(path):
    return os.path.join(os.path.dirname(os.path.realpath(__file__)), path)

def load_associated_company_schema(config):
    associated_company_schema = load_schema("companies", config)
...
def load_schema(entity_name, config):
    ...
    if entity_name in ["contacts", "companies", "deals"]:
        ...
        if entity_name in ["deals"]:
            v3_schema = get_v3_schema(entity_name, config)
    ...
    if entity_name == "contacts":
        schema['properties']['associated-company'] = load_associated_company_schema(config)
...
def load_discovered_schema(stream, config):
    schema = load_schema(stream.tap_stream_id, config)
    mdata = metadata.new()
...
def discover_schemas(config):
    ...
    for stream in STREAMS:
        ...
        schema, mdata = load_discovered_schema(stream, config)
    ...
    schema, mdata = load_discovered_schema(contacts_by_company, config)
